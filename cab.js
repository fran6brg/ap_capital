    ...
    for (letter = 0; letter < 26; letter++) {
        try {
            ...
            for (var nb_page = 1; nb_page <= nb_page_for_letter; nb_page++) {
                ...
                let rows = [];
                let cab_cards = [];
                cab_cards = await page.$$(params.slt.cabs.card);
                console.log("   nb cab_cards: ", cab_cards.length);
                /* -------------------------------- */
                for (let i = 0; i < cab_cards.length; i++)
                {
                    let href = await cab_cards[i].$eval('a', a => a.getAttribute('href'));
                    let name_addr = await (await cab_cards[i].getProperty('innerText')).jsonValue();
                    ...
                    row.id = 'c' + (letter + 10).toString(36) + ('0' + nb_page).slice(-2);
                    row.letter = (letter + 10).toString(36);
                    row.number = nb_page;
                    row.name = tab[0].trim();
                    row.address = tab[2].trim();
                    row.url = params.urls.main.slice(0, -1) + href;
                    // console.table(row);
                    rows.push(row);
                    index++;
                }
                console.log(chalk.green("   +" + rows.length + " cabs added to excel"));
                /* -------------------------------- */
